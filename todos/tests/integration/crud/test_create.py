...
@pytest.fixture()
def exemplary_event(exemplary_event_body: dict) -> dict:
    return {'body': json.dumps(exemplary_event_body)}
...
def test_should_return_bad_request_when_name_is_missing(dbsession: orm.Session, exemplary_event_body: dict) -> None:
...
def test_should_return_bad_request_when_description_is_missing(
        dbsession: orm.Session, exemplary_event_body: dict
) -> None:
    event_body = copy.deepcopy(exemplary_event_body)
